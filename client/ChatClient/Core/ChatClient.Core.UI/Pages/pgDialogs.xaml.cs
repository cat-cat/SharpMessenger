           ...
           lstDialogs.ItemTapped += async (sender, args) =>
            {
               Conversation item = args.Item as Conversation;
                if (item == null) return;
				await Navigation.PushAsync(new ChatPage() {BindingContext = new PrivateChatViewModel(item.Message.Opponent.Id!=Authorization.GetUser().Result.Id?item.Message.Opponent:item.Message.OwnerId)});
